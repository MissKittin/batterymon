...
case "${1}" in
	...
	'status')
		if "${ENABLE_WATCHDOG}"; then
			status_of_proc -p "${PIDFILE}" "$(readlink -f ${0})" 'batterymon' && exit 0 || exit $?
		else
	...
	'daemon')
		...
		if ! kill -0 "${DAEMON_PID}" > '/dev/null' 2>&1; then
			echo "$(date '+%Y-%m-%d %H:%M:%S') Daemon failed immediately, stopping batterymon watchdog" >> "${LOG}"
			exit 1
		fi
		...
		while :; do
			echo "$(date '+%Y-%m-%d %H:%M:%S') Restarting batterymon" >> "${LOG}"
			sleep 1
